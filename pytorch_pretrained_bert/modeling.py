...
class BertEmbeddings(nn.Module):
    ...
    def forward(self, vis_feats, vis_pe, input_ids, token_type_ids=None, context=None, cxt_modality_label=None,
                position_ids=None, max_len_img_cxt=200, prev_is_None=True):
...
class BertModel(PreTrainedBertModel):
    ...
    def forward(self, vis_feats, vis_pe, input_ids, token_type_ids=None, attention_mask=None, context=None,
                cxt_modality_label=None, output_all_encoded_layers=True, max_len_img_cxt=200):

        extended_attention_mask = self.get_extended_attention_mask(
            input_ids, token_type_ids, attention_mask)

        # hack to load vis feats
        embedding_output = self.embeddings(vis_feats, vis_pe, input_ids, token_type_ids, context=context,
                                           cxt_modality_label=cxt_modality_label, max_len_img_cxt=max_len_img_cxt)
...
class BertModelIncr(BertModel):
    ...
    def forward(self, vis_feats, vis_pe, input_ids, token_type_ids, position_ids, attention_mask, context=None,
        ...
        embedding_output = self.embeddings(
            vis_feats, vis_pe, input_ids, token_type_ids, position_ids=position_ids, context=context,
            cxt_modality_label=cxt_modality_label, max_len_img_cxt=max_len_img_cxt, prev_is_None=prev_is_None)
...
class BertForWebqa(PreTrainedBertModel):
    ...
    def __init__(
            self, config, fc7_weight_path: str, fc7_bias_path: str, num_labels=2, max_len_img_cxt=200, use_vinvl=False
    ):
        ...
        if use_vinvl:
            ...
            with open(fc7_weight_path, 'rb') as f:
                self.vis_embed[0].weight.data.copy_(torch.from_numpy(pickle.load(f)))
            with open(fc7_bias_path, 'rb') as f:
                self.vis_embed[0].bias.data.copy_(torch.from_numpy(pickle.load(f)))
    ...
    def forward(self, vis_feats=None, vis_pe=None, input_ids=None, token_type_ids=None, attention_mask=None,
        ...
        if context[0] in ['img', 'both'] and vis_feats.size()[-1] > 1:
            ...
            vis_pe = self.vis_pe_embed(
                vis_pe)  # image region positional encodings (NC1+NC2+ ... +NC_B, 100, hidden_size), NC = num_choices
        ...
        if do_filter_task[0]:
            ...
            def cross_entropy_with_logits_loss(prediction, target, logit_mask):
                ...
                m = lp * target * logit_mask.view(-1, num_choices, 1).repeat(1, 1,
                                                                             2)  # target.transpose --> batch_size x num_choices x 2
            ...
            if isinstance(cxt_modality_label, list): cxt_modality_label = torch.squeeze(
                torch.LongTensor(cxt_modality_label), 1)  # before flatten, it's like [[1], [1], [1], ...]
            ...
            def mlmloss_mask_and_normalize(loss, mask, drop_worst_ratio):
                ...
                keep_loss, keep_ind = torch.topk(loss.sum(-1), int(loss.size(0) * (1 - drop_worst_ratio)),
                                                 largest=False)
            ...
            if tokenizer is not None:
                ...
                print(sequence)
                print(
                    tokenizer.convert_ids_to_tokens([i for i in list(input_ids.detach().cpu().numpy()[0][:]) if i > 0]))
                print(tokenizer.convert_ids_to_tokens(
                    [i for i in list(masked_lm_labels.detach().cpu().numpy()[0]) if i > 0]))
                # print("\n")
                time.sleep(2)
            return masked_lm_loss, cls_loss
...
class BertForWebqaDecoder(PreTrainedBertModel):
    ...
    def forward(self, vis_feats, vis_pe, input_ids, token_type_ids, position_ids, attention_mask, context=None,
                cxt_modality_label=None, task_idx=None, sample_mode='greedy', tokenizer=None):
        if context[0] in ['img', 'both'] and vis_feats.size()[-1] > 1:
            vis_feats = self.vis_embed(
                vis_feats)  # image region features (NC1+NC2+ ... +NC_B, 100, hidden_size), NC = num_choices
            vis_pe = self.vis_pe_embed(
        ...
        if isinstance(cxt_modality_label, list): cxt_modality_label = torch.squeeze(
            torch.LongTensor(cxt_modality_label), 1)  # before flatten, it's like [[1], [1], [1], ...]
        cxt_modality_label = [i for i in range(len(cxt_modality_label)) if cxt_modality_label[i]]
        if context[0] in ['img', 'both']: assert np.array(cxt_modality_label).size == vis_feats.size(0) == vis_pe.size(
            0)
        # print("cxt_modality_label = ", cxt_modality_label)

        if self.search_beam_size > 1:
            return self.beam_search(vis_feats, vis_pe, input_ids, token_type_ids, position_ids, attention_mask, context,
                                    cxt_modality_label, task_idx)
        ...
        curr_ids = input_ids
        mask_ids = input_ids[:,
                   :1] * 0 + self.mask_word_id  # same size as input_ids[:, :1], filled with mask_word_id # batch_size x 1
        ...
        while next_pos < output_length:
            ...
            new_embedding, new_encoded_layers, _ = \
                self.bert(vis_feats, vis_pe, x_input_ids, curr_token_type_ids, curr_position_ids, curr_attention_mask,
                          context[0], cxt_modality_label, prev_embedding=prev_embedding,
                          prev_encoded_layers=prev_encoded_layers,
            ...
            if tokenizer is not None:
                ...
                print(sequence)
                print(tokenizer.convert_ids_to_tokens(
                    [i for i in list(input_ids.detach().cpu().numpy()[0][202:]) if i > 0 and i != 102]))
                # print("\n")
                time.sleep(2)
        ...
        # print(output_probs[-1].size())
        return [{s.item(): seq} for s, seq in
                zip(output_probs[-1].detach().cpu().numpy(), torch.cat(output_ids, dim=1).detach().cpu().numpy())]
        # return torch.cat(output_ids, dim=1), torch.cat(output_probs, dim=1)

    def beam_search(self, vis_feats, vis_pe, input_ids, token_type_ids, position_ids, attention_mask, context,
                    cxt_modality_label, task_idx=None):
        ...
        while next_pos < output_length:
            ...
            if len(total_scores) == 0:
                ...
                kk_scores += last_eos * (
                    -10000.0) + last_seq_scores  # don't consider beams that already have <eos> in the last step
            ...
            step_back_ptrs.append(
                back_ptrs)  # record prev_step beam id. get the actual word_id from corresponding pos in step_ids
            ...
            if prev_embedding is None:
                ...
                prev_embedding = torch.cat(
                    (prev_embedding, new_embedding[:, :-1, :]),
                    dim=1)  # new_embedding[:, :-1, :].size() = batch_size*K x 1 x emb_dim
